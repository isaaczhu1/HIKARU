"""Targeted tests for the HanabiGym2P observation pipeline."""
...
def test_observation_space_matches_vector_length():
    env = HanabiGym2P(seed=0, obs_conf="minimal")
    assert env.observation_space["obs"].shape == (VEC_LEN,)
...
def test_legal_mask_marks_all_stub_moves(monkeypatch):
    ...
    env = HanabiGym2P(seed=0, obs_conf="minimal", players=2, colors=2, ranks=2, hand_size=2)
    obs, _ = env.reset()
...
def test_hint_actions_include_target_offset(monkeypatch):
    ...
    obs, reward, terminated, truncated, _ = env.step(hint_id)
