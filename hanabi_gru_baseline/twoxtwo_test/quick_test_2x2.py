...
def main():
    ...
    # Roll a few random-legal steps to verify masking & prev_other_action wiring
    ep_ret = 0.0
    steps = 0
    t0 = time.time()
    for _ in range(50):
        ...
        steps += 1
        ep_ret += r
        print(f"step={steps:02d} seat={seat} prev_other={prev if prev != SENTINEL_NONE else 'NONE'} "
              f"act={a} legal={len(legal)} r={r} score={info['score']} done={term or trunc}")
        if term or trunc:
