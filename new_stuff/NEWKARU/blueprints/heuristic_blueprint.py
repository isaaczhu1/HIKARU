...
class HeuristicBlueprint:
    ...
    def act(self, observation: HanabiObservation) -> pyhanabi.HanabiMove:
        """Selects an action consistent with the heuristic rules."""
    ...
    def _hint_covered_indices(self, obs: HanabiObservation, move_dict: Dict[str, object]) -> List[int]:
        ...
        covered: List[int] = []
        if move_dict["action_type"] == "REVEAL_COLOR":
            target_color = move_dict["color"]
            for idx, card in enumerate(partner_hand):
                if card.get("color") == target_color:
                    covered.append(idx)
        elif move_dict["action_type"] == "REVEAL_RANK":
            target_rank = move_dict["rank"]
            for idx, card in enumerate(partner_hand):
                if card.get("rank") == target_rank:
                    covered.append(idx)
        return covered

    def _record_rule_usage(self, rule: str) -> None:
